...
除非另有说明，本讲义一律采用整数系数的奇异同调。

设 $M,M'$ 是光滑 $(r+s)$-流形 $V$ 中维数分别为 $r,s$ 的光滑子流形，二者横截相交于 $p_1,\ldots,p_k$。假设 $M$ 已\Term{定向}{oriented}，$M'$ 在 $V$ 中的\Term{法丛}{normal bundle} $\nu(M')$ 也已定向。在 $p_i$ 处，取张成 $T_{p_i}M$ 的正向 $r$-标架 $\xi_1,\ldots,\xi_r$。由于交是横截的，这些向量在 $p_i$ 处给出 $\nu(M')$ 纤维的一组基。

\Definition{6.1}若 $\xi_1,\ldots,\xi_r$ 在 $p_i$ 处给出 $\nu(M')$ 纤维的正向基，则定义 $M,M'$ 在 $p_i$ 处的\emph{\Term{相交数}{intersection number}}为 $+1$；若给出负向基，则定义为 $-1$。$M$ 与 $M'$ 的相交数 $M'\mathbin{\boldsymbol\cdot}M$，定义为各交点处相交数之和。
...
令 $\alpha$ 为 $H_0(M')\cong\Z$ 的典范生成元，$[M]\in H_r(M)$ 为定向生成元。
...
\RemarkLabel{1}\label{rem:6-4-1}水平集 $V=f^{-1}(\tfrac12)$ 也单连通。事实上，两次应用 van Kampen 定理（Crowell 与 Fox~\cite[p.~63]{ref17}）可得
\[
  \pi_1(V)\cong
  \pi_1\bigl(D_R^{n-\lambda}(p)\cup V\cup D_L^{\lambda+1}(q)\bigr);
\]
这里用到了 $\lambda\geq2$、$n-\lambda\geq3$。而由\ThmRef{3.4}，包含
\[
  D_R(p)\cup V\cup D_L(q)\subset W
\]
是同伦等价。合并两点便知 $\pi_1(V)=1$。
...
设 $p,q\in M\cap M'$ 的相交数相反，并存在一条在 $V$ 中可缩的闭路 $L$：它先沿 $M$ 中从 $p$ 到 $q$ 的光滑嵌入弧，再沿 $M'$ 中从 $q$ 到 $p$ 的光滑嵌入弧；两条弧都避开
...
\]
时，分别在 $M\setminus S$ 与 $M'\setminus S$ 上取完备 Riemann 度量，应用 Hopf--Rinow 定理（见 Milnor~\cite[p.~62]{ref04}），可在 $M$ 中找到从 $p$ 到 $q$ 的光滑嵌入弧，在 $M'$ 中找到从 $q$ 到 $p$ 的光滑嵌入弧；它们组成避开 $S$ 的闭路 $L$。若 $V$ 单连通，$L$ 当然可缩。
...
\begin{enumerate}[label=\arabic*)]
  \item 对相应的联络（见 Milnor~\cite[p.~44]{ref04}），$M,M'$ 都是 $V$ 的\Term{全测地子流形}{totally geodesic submanifold}；即 $V$ 中一条测地线若在某点与 $M$ 或 $M'$ 相切，就全部落在相应的子流形中；
...
\Proof 设 $\omega$ 是 $T$ 中在某点 $z\in M$ 与 $M$ 相切的测地线。$A$ 在新度量下是等距映射，故把测地线映到测地线。$M$ 是 $A$ 的不动点集，所以 $A(\omega)$ 与 $\omega$ 在 $A(z)=z$ 处有相同的切向量。由测地线的唯一性，$A$ 在 $\omega$ 上恒等，因而 $\omega\subset M$。断言得证。
\EndProof
...
\[
  \tau(p),\tau(q),\tau'(p),\tau'(q)
\]
为弧 $C,C'$（均从 $p$ 定向到 $q$）在 $p,q$ 处的单位切向量。$C$ 可缩，所以其上与 $M$ 正交的向量丛平凡。由此沿 $C$ 构造与 $M$ 正交的单位向量场，使它在 $N_p\cap C$ 上等于 $\tau'(p)$ 的平行移动，在 $N_q\cap C$ 上等于 $-\tau'(q)$ 的平行移动。在模型中构造相应的向量场（见\FigRef{6.5}）。
...
给定闭路 $L$ 与 $\phi_2(S)$ 同伦，所以后者在 $V$ 中可缩。下面说明它事实上在 $V\setminus(M\cup M')$ 中也可缩。
...
\]
在 $T_pM$ 中为正向。沿 $C$ 平行移动这 $r-1$ 个向量，得到光滑向量场 $\xi_1,\ldots,\xi_{r-1}$。平行移动保持内积（见 Milnor~\cite[p.~48]{ref04}），故满足 (1)；沿全测地子流形中的曲线作平行移动，会把切向量仍送到该子流形的切向量（见 Helgason~\cite[p.~80]{ref14}），故满足 (2)。对\LemRef{6.8} 中所构造的度量，(2) 也可由 $M$ 的管状邻域上的对径等距映射 $A$ 直接推出。由连续性，标架 $\tau,\xi_1,\ldots,\xi_{r-1}$ 沿 $C$ 处处是 $TM$ 中的正向标架。
...
$\nu',\zeta_1,\ldots,\zeta_{r-1}$ 在 $\nu(M')$ 中为正向的向量丛是平凡丛，纤维为连通的 $SO(r-1)$。所以可把 $\xi_1,\ldots,\xi_{r-1}$ 延拓为 $C\cup C'$ 上满足 (1)、(2) 的光滑标架场。
...
\[
  ...
  =V_{r-1}(\R^{r+s-2}).
\]
我们已在 $C\cup C'$ 上构造了该丛的光滑截面。把它投影到纤维，得到 $C\cup C'$ 到上述 Stiefel 流形的光滑映射。因 $s\geq3$，该纤维单连通（见 Steenrod~\cite[p.~103]{ref18}），所以映射可连续延拓到 $U'$；由\LemRef{6.11}，又可取光滑延拓。于是 $\xi_1,\ldots,\xi_{r-1}$ 在整个 $U'$ 上满足 (1)、(2)。
