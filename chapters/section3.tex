...
\Definition{3.1}设 $f$ 是三元组 $(W^n;V,V')$ 上的 Morse 函数。若 $W^n$ 上的\Term{向量场}{vector field} $\xi$ 满足：
...
\]
视为同一对象，其中 $\operatorname{Id}_i:V_i\to V_i$ 是\Term{恒等映射}{identity map}。
...
\Proof $M$ 的各分支可被 $W$ 中两两不交的开集覆盖，所以只需考虑 $M$ 只有一个分支的情形。取 $M$ 在 $W^n\setminus\partial W^n$ 中的开邻域 $U$，使 $\overline U$ 紧，并位于一个删去 $M$ 后分成两部分的邻域中。于是 $U$ 分解成两个子流形 $U_1,U_2$，且 $U_1\cap U_2=M$ 是二者的边界。

与\LemRef{2.6} 的证明一样，利用坐标覆盖和单位分解构造光滑映射 $\phi:\overline U\to\R$，使 $d\phi$ 在 $M$ 上不为零，并且
\[
  \phi<0\text{ 于 }\overline U\setminus U_1,\qquad
  \phi=0\text{ 于 }M,\qquad
  \phi>0\text{ 于 }\overline U\setminus U_2.
\]
可取 $M$ 的开邻域 $V$，$\overline V\subset U$，使 $\phi$ 在 $V$ 中没有临界点。令 $2\varepsilon''>0$ 为 $\phi$ 在紧集 $\overline U_1\setminus V$ 上的最小上界，令 $2\varepsilon'<0$ 为 $\phi$ 在紧集 $\overline U_2\setminus V$ 上的最大下界。则 $\phi^{-1}[\varepsilon',\varepsilon'']$ 是 $V$ 中的紧 $n$ 维带边界子流形，且 $\phi$ 是其上的 Morse 函数。应用\ThmRef{3.4}\footnote{译注：原文此处误引“定理 3.1”；应为无临界点情形的乘积配边定理，即定理 3.4。}，$\phi^{-1}(\varepsilon',\varepsilon'')$ 就是 $M$ 在 $V$ 中、从而也是在 $W$ 中的双侧领状领域。
\EndProof
...
\Definition{3.11}设 $V$ 是 $(n-1)$ 维流形，$\phi:S^{\lambda-1}\times\mathring D^{n-\lambda}\to V$ 是嵌入。以 $\chi(V,\phi)$ 表示如下\Term{商流形}{quotient manifold}：从不交并
\[
...
\Corollary{3.15}$H_*(W,V)$ 在维数 $\lambda$ 上同构于 $\Z$，在其他维数上为零。$D_L$ 表示 $H_\lambda(W,V)$ 的一个\Term{生成元}{generator}。
...
\]
即 $D_L$ 的 $1/10$ 柱状邻域。先定义从 $W$ 到 $V\cup C$ 的\Term{形变收缩}{deformation retraction} $r_t$，再定义从 $V\cup C$ 到 $V\cup D_L$ 的形变收缩 $r'_t$，二者复合即为所需收缩。
