...
\Remark 这一证明源于 M. Morse~\cite{ref11,ref32}，颇为艰深。不计技术性的\ThmRef{5.6}，证明见\TranslatedPageRange{proof:5-4-start}{proof:5-4-local-end}。\footnote{译注：原文称证明占下文十页；在本译本中见\TranslatedPageRange{proof:5-4-start}{proof:5-4-local-end}。}
...
\par\medskip\noindent\phantomsection\label{assump:5.5}\label{proof:5-4-start}\textbf{预备假设 5.5.}\quad
...
\phantomsection\label{proof:5-4-local-end}在预备假设成立时，第一消去\ThmRef{5.4}\footnote{译注：原文此处两次将第一消去定理误记为 5.5，应为 5.4；5.5 是预备假设的编号。}已证。为证明一般情形，还须证明：
...
\Remark 余下的证明见\TranslatedPageRange{assertion:5-6-page55}{proof:5-6-end}，分两步进行：\footnote{译注：原文称证明占本节最后十二页；在本译本中见\TranslatedPageRange{assertion:5-6-page55}{proof:5-6-end}。}先把问题归结为一个技术性引理（\ThmRef{5.6}），再证明该引理。
...
由类梯度向量场的\DefRef{3.1}，在临界点 $p,p'$ 各自的某个坐标系中，$f$ 都可写成指标相应的二次型 $\pm x_1^2\pm\cdots\pm x_n^2$，而 $\xi$ 的坐标为 $(\pm x_1,\ldots,\pm x_n)$。由此容易验证，存在水平值
\[
...
\begin{enumerate}[label=(\alph*)]
  ...
  \item 令 $p_i=T\cap f^{-1}(b_i)$，$i=1,2$。则 $g_1(L_1)$ 是 $f^{-1}[a_1,b_1]$ 中线段 $pp_1$ 的邻域，而 $g_2(L_2)$ 是 $f^{-1}[b_2,a_2]$ 中线段 $p_2p'$ 的邻域（见\FigRef{5.5}）。
\end{enumerate}
...
\[
  p_1=h_0^{-1}h(p_1)
\]
处保持\Term{定向}{orientation}，并使 $h_0^{-1}h(S_R(b_1))$ 与 $S_R(b_1)$ 在 $p_1$ 处同 $S_L(b_1)$ 的\Term{相交数}{intersection number}相同（均为 $+1$ 或均为 $-1$）。相交数的定义见\SecRef{6}。下述局部定理给出所需同痕。
...
\par\medskip\noindent\textit{\ThmRef{5.6} 的证明（续）.}\quad 回到\ThmRef{5.6} 的证明，以 $\bar h_t$ 表示上述延拓。若 $\bar h_t$ 使 $\R^a$ 的像与 $\R^b$ 产生新的交点，\ThmRef{5.6} 的条件 (III) 就会失效，如\FigRef{5.9}所示。
...
\end{enumerate}
于是 $\bar h_t$ 在 $[t_0,1]\times\R^n$ 上处处满足 (\S)。
...
\[
  ...
  \end{cases}
\]
常数 $k/K$ 只依赖于 $h_t$，所以有限次复合上述同痕便得到所需的光滑同痕。\ThmRef{5.6} 得证，从而\NamedRef{claim:5.6}{断言 6}（见\TranslatedPage{assertion:5-6-page55}）\OriginalPageNote{55}{assertion:5-6-page55}成立，第一消去定理也在一般情形下得证。\label{proof:5-6-end}
\EndProof
