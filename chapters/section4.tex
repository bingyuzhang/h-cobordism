...
\[
  ...
  \operatorname{index}(c')=\operatorname{index}(d).
\]
此时称复合 $cc'$\emph{可以重排}。何时能够重排？

回忆一下，表示 $cc'$ 的三元组 $(W;V_0,V_1)$ 上存在 Morse 函数
...
\Theorem{4.1（初步重排定理）}设三元组 $(W;V_0,V_1)$ 上的 Morse 函数 $f$ 恰有两个临界点 $p,p'$。若存在 $f$ 的类梯度向量场 $\xi$，使由趋向 $p$ 或从 $p$ 出发的轨线上的点连同 $p$ 组成的紧集 $K_p$ 与相应的紧集 $K_{p'}$ 不交，则在 $f(W)=[0,1]$ 时，对任意 $a,a'\in(0,1)$，存在新的 Morse 函数 $g$，满足：
...
对每个固定的 $q\in V$，曲线 $\phi(t,h_t(q))$ 给出新向量场的一条轨线，它在 $V$ 中从
\[
  \phi(\tfrac12,h(q))=h(q)
\]
相交。于是，$p$ 在 $f^{-1}(a)$ 中的右球面 $\phi(\{a\}\times S_R)$ 沿新轨线被送到 $h(S_R)$；故 $h(S_R)$ 正是 $p$ 的新右球面。左球面 $S'_L$ 未变。记新右球面为 $\overline S_R=h(S_R)$，则
\[
  \overline S_R\cap S'_L=h(S_R)\cap S'_L=\varnothing.
\]
\ThmRef{4.4} 证毕。
\EndProof
...
由\CorRef{2.10}\footnote{译注：原文此处误引为推论 2.11；原书对应结果的编号为 2.10。}，每个配边都可分解为有限多个初等配边的复合。结合初步重排\ThmRef{4.1}、\NamedRef{stmt:4.2}{4.2} 与\ThmRef{4.4}，便得：
