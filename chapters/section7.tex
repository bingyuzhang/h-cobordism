...
\]
在 $T_xW$ 中为正向，则称前一标架为正向；这里 $\nu$ 是 $x$ 处任一切于 $W$、不切于 $X$ 且指向 $W$ 外部的向量（即 $X$ 的\Term{外法向量}{outward normal vector}）。

也可等价地指定 $[X]\in H_{n-1}(X)$ 为 $X$ 的诱导定向生成元：在偶对 $(W,X)$ 的\Term{正合序列}{exact sequence}中，$[X]$ 是 $W$ 的定向生成元 $[W]\in H_n(W,X)$ 在\Term{边界同态}{boundary homomorphism}
\[
  \partial:H_n(W,X)\longrightarrow H_{n-1}(X)
\]
下的像。

\Remark 紧流形 $M^n$ 的定向既可用\Term{切丛}{tangent bundle}的有序标架指定，也可用生成元 $[M]\in H_n(M;\Z)$ 指定；二者之间有自然的一一对应（参见 Milnor~\cite[p.~21]{ref19}）。在此对应下，上述两种 $\partial W$ 定向彼此等价。下文一律采用第二种方式，证明从略。
...
\begin{tikzcd}[column sep=4.0em,row sep=2.15em]
  & H_\lambda(M)
      \arrow[d]
      \arrow[rrd,"h_0","{\times(S_R\mathbin{\boldsymbol\cdot}M)}"']
      \arrow[ldddd,"h"']
  && {} \\
  {} & H_\lambda(V') \arrow[d,"\iota_*"'] \arrow[rr]
...
\]
最后一个同构留给读者验证（见 Milnor~\cite[p.~9]{ref19}），从而得到所需结论。
\EndProof
...
\[
  c_{n-\mu}c_{n-\mu+1}\circ\cdots\circ c_n,
...
由\ThmRef{7.4}\footnote{译注：原文此处误引为 7.1；计算同调的结果是定理 7.4。}，
\[
  H_\lambda(C_*)\cong H_\lambda(W,V),
  \qquad
...
\]
因为两个自由阿贝尔链复形的同调若同构，其对偶上链复形的上同调也同构；这是\Term{万有系数定理}{universal coefficient theorem}的直接推论。合并以上各式即得
...
由\CorRef{3.5}，在 $V_0$ 右侧取嵌入 $W$ 的乘积邻域 $V_0\times[0,1]$，使其中没有临界点且 $V_0\times\{0\}=V_0$。依\LemRef{4.7}，用同痕 $F_t$ 修改此邻域内的向量场 $\xi$，得到 $W$ 上的新向量场 $\xi'$。
...
\]
因为 $\xi'=\xi$ 在乘积邻域 $V_0\times[0,1]$ 的左侧相同。二者在该邻域右侧也相同，所以新左圆盘 $D'_L(p_1)$ 与 $D_R(p_1)$ 只在 $p_1$ 处相交，且
\[
...
\[
  c=c_2c_3\circ\cdots\circ c_{n-2},
\]
...
\[
  ...
  =c'_\lambda\circ c_pc_qc'_{\lambda+1},
\]
其中 $c_p,c_q$ 分别恰有一个临界点 $p,q$。令 $V_0$ 为二者之间的水平流形。容易验证，$c_pc_q$ 及其两个端流形都单连通（比较注记 1，见\TranslatedPage{rem:6-4-1}）。\OriginalPageNote{70}{rem:6-4-1}

由于
\[
  \partial b^{\lambda+1}_1=z^\lambda_1,
\]
$V_0$ 中的球面 $S_R(p),S_L(q)$ 的相交数为 $+1$。所以第二消去定理（\ThmRef{6.4}）或\CorRef{6.5} 表明，$c_pc_q$ 是乘积配边；还可在其内部修改 $f'$ 及类梯度向量场，使 $f'$ 在那里没有临界点。如此反复，即可消去全部临界点。最后由\ThmRef{3.4}，$(W;V,V')$ 是乘积配边。
\EndProof
