...
\Remark 事实上，只需 $n\geq2m+2$，\ThmRef{8.4} 仍成立（见~\cite{ref10}）。

\par\medskip\noindent\textit{\ThmRef{8.1} 指标 1 情形的证明（续）.}\quad 注意 $V_{2+}$ 总是单连通。事实上，包含映射 $V_{2+}\subset W$ 可分解为一串包含映射，交替对应于添附胞腔和同伦等价（见\CorRef{3.15}）。向左时添附的胞腔维数为 $n-2,n-1$，向右时则为 $3,4,\ldots$。所以，$W$ 连通蕴含 $V_{2+}$ 连通，并且
\[
  \pi_1(V_{2+})=\pi_1(W)=1
\]
...
\[
  f|_U=x_n
\]
（比较\LemRef{2.11} 的证明）。应用\LemRef{8.2}，在 $U$ 的一个紧子集上修改 $f$，添入一对辅助临界点 $q,r$，使
\[
  f(q)<f(r),
\]
...
令 $S_2$ 为 $q$ 在 $V_{2+}$ 中的左 $1$-球面。因 $V_{2+}$ 单连通，\ThmRef{5.8}、\ThmRef{8.4} 表明，恒等映射 $V_{2+}\to V_{2+}$ 有一个同痕把 $S_2$ 映到 $S_1$。所以在 $V_{2+}$ 右侧调整 $\xi$ 后（见\LemRef{4.6}），可使 $q$ 在 $V_{2+}$ 中的左球面变为 $S_1$。于是 $q$ 在 $V_{1+}$ 中的左球面就是 $S$；依构造，它与 $p$ 的右球面恰在一点横截相交。
