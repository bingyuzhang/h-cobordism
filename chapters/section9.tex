...
\[
  ...
  \cong H_*(W,D_0)=0.
\]
所以配边 $(W^n;\varnothing,V)$ 是 $(D_0;\varnothing,\partial D_0)$ 与积配边 $(W\setminus\Int D_0;\partial D_0,V)$ 的复合。由\ThmRef{1.4}，$W$ 微分同胚于 $D_0$。
\EndProof

\Proposition{B（维数不小于 5 的广义 Poincaré 猜想；见 Smale~\cite{ref22}）}设 $M^n$ 是闭单连通光滑流形，$n\geq5$，并且具有 $n$-球面 $S^n$ 的整数同调。则 $M^n$ 同胚于 $S^n$；若 $n=5$ 或 $6$，则 $M^n$ 微分同胚于 $S^n$。

\Corollary{9.4}\footnote{译注：原书此推论未标编号；依本节的连续编号应为\CorRef{9.4}。}若闭光滑流形 $M^n$（$n\geq5$）是\Term{同伦 $n$-球面}{homotopy $n$-sphere}，即具有 $S^n$ 的同伦型，则 $M^n$ 同胚于 $S^n$。
...
\Theorem{9.5（Kervaire 与 Milnor~\cite{ref25}；Wall~\cite{ref26}）}\footnote{译注：原书此处误作“定理 9.1”；依本节的连续编号应为\ThmRef{9.5}。}设 $M^n$ 是闭单连通光滑流形，并具有 $n$-球面 $S^n$ 的同调。若 $n=4,5$ 或 $6$，则 $M^n$ 是某个紧致、可缩光滑流形的边界。
