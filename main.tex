% !TeX program = xelatex
% !TeX encoding = UTF-8
\documentclass[UTF8,zihao=-4,openright,fontset=fandol]{ctexbook}
...
\newcommand{\EndProof}{\qed\par}
% 为以行间公式结束的证明建立独立 QED 栈；兼容正文中的分段证明。
\newenvironment{MilnorProofEnd}{\pushQED{\qed}}{\popQED}
...
\newcommand{\TranslatedPageRange}[2]{%
  \ifnum\getpagerefnumber{#1}=\getpagerefnumber{#2}%
    \pageref{#1}~页%
  \else
    \pageref{#1}--\pageref{#2}~页%
  \fi}
\newcommand{\TranslatedPage}[1]{\pageref{#1}~页}
...
\newcommand{\OriginalPageRangeNote}[3]{%
  ...
  \newcommand{\Source}[1]{\noindent\textit{原书位置：#1。}\par}
\input{chapters/tikzfigures}
...
\mainmatter
\input{chapters/translation-notes}
...
\input{chapters/section9}
\input{chapters/references}
\input{chapters/glossary}
\input{chapters/potential-mistakes}
